...
PROJECT_ROOT=$(grep "^  root:" src/workflow/config/paths.yaml | cut -d' ' -f4)
SRC_DIR="${PROJECT_ROOT}/src"
PUBLIC_DIR="${PROJECT_ROOT}/public"

# Alternative: Use yq for complex path resolution
if command -v yq &> /dev/null; then
  PROJECT_ROOT=$(yq eval '.project.root' src/workflow/config/paths.yaml)
  SRC_DIR=$(yq eval '.directories.src' src/workflow/config/paths.yaml)
fi
...
const config = yaml.load(readFileSync('src/workflow/config/paths.yaml', 'utf8'));
