...
# Version: 2.1.3 (Phase 3 - Adaptive)
################################################################################
...
build_step10_code_quality_prompt() {
    local total_files="$1"
    local js_files="$2"
    local html_files="$3"
    local css_files="$4"
    ...
    cat << EOF
You are a code quality validation specialist. Analyze the codebase and provide recommendations for improvement.
...
## Your Task
1. Review automated linting and quality issues
...
6. Suggest refactoring opportunities for large files
7. Provide specific, actionable recommendations

Focus on critical issues that impact code quality, maintainability, and reliability.
EOF
}

export -f build_step10_code_quality_prompt
