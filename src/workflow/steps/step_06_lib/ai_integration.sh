...
# Usage: build_test_review_prompt_step6 <test_count> <test_files> <coverage_summary> <issues>
# Returns: Complete AI prompt
build_test_review_prompt_step6() {
    local test_count="$1"
    local test_files="$2"
...
# Usage: enhance_test_review_prompt_step6 <base_prompt> <language>
# Returns: Enhanced prompt
enhance_test_review_prompt_step6() {
    local base_prompt="$1"
...
# Usage: execute_test_review_step6 <prompt> <log_file>
# Returns: 0 on success, 1 if skipped
execute_test_review_step6() {
    local prompt="$1"
    local log_file="$2"
    ...
    if command -v execute_copilot_prompt &>/dev/null; then
        execute_copilot_prompt "$prompt" "$log_file" "step06" "test_engineer"
    else
    ...
    if command -v extract_and_save_issues_from_log &>/dev/null; then
        extract_and_save_issues_from_log "6" "Test_Review" "$log_file"
    fi
...
# Usage: run_ai_test_review_workflow_step6 <test_count> <test_files> <coverage_summary> <issues>
# Returns: 0 on success, 1 if skipped
run_ai_test_review_workflow_step6() {
    local test_count="$1"
    local test_files="$2"
    ...
    # Build prompt
    local prompt
    prompt=$(build_test_review_prompt_step6 "$test_count" "$test_files" "$coverage_summary" "$issues")
    
    # Enhance with language conventions
    prompt=$(enhance_test_review_prompt_step6 "$prompt" "${PRIMARY_LANGUAGE:-javascript}")
    
    # Create log file
    local log_timestamp
    log_timestamp=$(date +%Y%m%d_%H%M%S_%N | cut -c1-21)
    local log_file="${LOGS_RUN_DIR:-./logs}/step6_copilot_test_review_${log_timestamp}.log"
    
    # Execute review
    execute_test_review_step6 "$prompt" "$log_file"
}
...
export -f build_test_review_prompt_step6
export -f enhance_test_review_prompt_step6
export -f execute_test_review_step6
export -f run_ai_test_review_workflow_step6
