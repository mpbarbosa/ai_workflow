...
# Version: 2.2.3 (Phase 3 - Adaptive + Regression Prevention)
################################################################################
...
# Build AI prompt for test execution analysis
# Args: $1=test_exit_code $2=tests_total $3=tests_passed $4=tests_failed
#       $5=execution_summary $6=test_output $7=failed_test_list
build_step8_test_exec_prompt() {
    ...
    cat << EOF
You are a test execution analysis specialist. Analyze the test results and provide actionable recommendations.
...
2. Determine if failures are:
   ...
   - Test code issues (flaky tests, outdated assertions)
   - Environment/configuration problems
3. Prioritize failures by severity and impact
4. Provide specific recommendations for each failure
5. Suggest any test improvements or refactoring needs

Focus on actionable insights that help developers quickly resolve issues.
EOF
}

export -f build_step8_test_exec_prompt
