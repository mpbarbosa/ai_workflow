...
detect_version_pattern() {
    ...
    # Check for version in header comments FIRST (more specific pattern)
    # This ensures shell scripts with "# Version: X.Y.Z" are handled correctly
    ...
    # Check for generic semver pattern (X.Y.Z) in code/config files
...
extract_current_version() {
    ...
    case "$pattern_type" in
        ...
        date)
            ...
            ;;
        semver_comment)
            # Extract version only from lines with "Version:" keyword
            grep -E '^#.*[Vv]ersion:?\s+[0-9]+\.[0-9]+\.[0-9]+' "$file" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1
            ;;
        *)
            echo ""
            ;;
