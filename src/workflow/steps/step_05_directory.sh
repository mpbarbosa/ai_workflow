...
# Version: 2.3.3 (Added incremental analysis optimization for client_spa)
################################################################################
...
# Build AI prompt for directory structure validation
# Args: $1=dir_count $2=change_scope $3=missing_critical $4=undocumented_dirs
#       $5=doc_structure_mismatch $6=structure_issues_content $7=dir_tree
build_step5_directory_prompt() {
    local dir_count="$1"
    ...
    cat << EOF
You are a directory structure validation specialist. Analyze the project directory structure and provide recommendations.
...
## Your Task
1. Analyze the directory structure for best practices
2. Identify any architectural concerns
3. Suggest improvements for organization
4. Validate that structure matches project type and scale
5. Recommend any reorganization needed

Focus on critical issues that impact maintainability and scalability.
EOF
}

export -f build_step5_directory_prompt
